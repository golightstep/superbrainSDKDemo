...
try:
    ...
    print("Warning: CrewAI dependencies missing. Running in ANALYTICS ONLY mode.")
    class TripAgents:
  ...
  def run(self):
    ...
    if DEPENDENCIES_AVAILABLE:
        if not os.getenv("OPENAI_API_KEY"):
            ...
            result = f"Traditional Trip Plan for {self.cities} focusing on {self.interests}. " * 5
            time.sleep(1.0)
        else:
            try:
                result = crew.kickoff()
    ...
    # Vanilla Simulation: 
    # Each of the 3 agents re-processes the entire prompt + history
    prompt_tokens = int(initial_words * 1.3)
    total_tokens = (prompt_tokens * 3) + int(result_words * 1.5)
    
    # Capture metrics (Simulating a slower traditional state exchange)
    time.sleep(0.5) # Simulate IO overhead of traditional serialization/db
    end_time = time.perf_counter()
    execution_time = (end_time - start_time) * 1000 # ms
    
    metrics = {
        "execution_time_ms": execution_time,
        "state_sharing_latency_us": 150000.0,
...
if __name__ == "__main__":
    ...
    if sys.stdin.isatty() and os.getenv("RUN_INTERACTIVE"):
        print("## Welcome to Trip Planner Crew (Vanilla)")
        print('-------------------------------')
